 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`acmcore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`acmcore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'acmcore-' + name : 'acmcore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/acmcore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external acmcore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:acmcore-lib -o acmcore-lib.js';
    }
